...
================================================================
- Google News RSS로 en-US / ko-KR / ja-JP / zh-CN 4개 지역 키워드 뉴스 수집
...
사용 전 준비 (기본, API 키 불필요):
    pip install feedparser requests python-dateutil deep-translator --break-system-packages
...
KEYWORD_GROUPS = {
    ...
    "바이오의약품 전반": ["Biologics", "Biopharmaceuticals", "recombinant DNA technology", "바이오의약품"],
    "세포유전자치료제": ["CAR-T", "CGT", "cell therapy", "gene therapy", "AAV", "stem cell", "줄기세포", "유전자치료제", "cancer vaccine"],
    "항체/치료제 모달리티": ["monoclonal antibody", "antibody", "ADC", "Abs biologics", "GLP-1 agonist", "항체"],
    "백신/톡신": ["vaccine", "botulinum toxin"],
    "규제/인허가": ["IND FDA", "FDA approval biologics", "PMDA approval", "BLA approval"
                 "CDE China drug approval", "MFDS", "HHS biologics policy", "MHRA policy", "guideline"],
}

# 검색 언어/지역: 미국, 한국, 일본(PMDA), 중국(CDE)
LANG_REGIONS = [
...
def translate_title_to_ko(title, lang):
    """원문 제목을 한국어로 번역 (API 키 불필요, deep-translator의 무료 Google Translate 사용).
    이미 한국어면 번역하지 않고 그대로 반환. 실패하면 None 반환 (원문만 표시됨)."""
...
def enrich_with_translations(articles):
    ...
    for a in articles:
        a["title_ko"] = translate_title_to_ko(a["title"], a["lang"])
    return articles
...
def build_markdown(articles):
    ...
    lines.append(f"(최근 {LOOKBACK_HOURS}시간 이내, 총 {len(articles)}건 · 4개 지역: 미국/한국/일본/중국)\n")
...
<div class="wrap">
  ...
  {body}
  <footer>매일 자동 수집 · Google News RSS 기반 · 무료 번역(deep-translator)</footer>
</div>
</body>
</html>"""
    return html
...
def main():
    print("[INFO] 뉴스 수집 시작 (미국/한국/일본/중국)...")
